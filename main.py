...
ANIMATION functions (handle ALL self.play/self.wait for their duration, call once with full chunk duration). EVERY function below returns the SAME shape: (group, primary_mobject) -- group is a VGroup of everything the call added (only useful if you need to FadeOut the whole thing later), primary_mobject is the single most relevant object (the counter text, the chart group, the bars, etc.) if you need to reference it afterward. This is uniform across every function in this library, no exceptions:
  fm_animate_counter(scene, start_val, end_val, label_text, accent_color=BRAND_GOLD, prefix="$", suffix="", duration=3.0, position=None, value_size=130, label_size=38)
    Counting number via ValueTracker+always_redraw. Zero LaTeX. Auto-varies its layout call to call so repeated counters in a video don't all look identical.
  ...
  fm_animate_gauge(scene, value, max_val, label_text, accent_color=BRAND_GREEN, duration=3.0, position=None, radius=2.0)
    Arc gauge (gray track + colored fill).
  ...
  fm_animate_donut(scene, percentage, label_text, accent_color=BRAND_GREEN, duration=3.0, position=None)
    Donut ring with pct text inside.
    Example: fm_animate_donut(self, 67, "Living Paycheck to Paycheck", BRAND_RED, duration=3.2)
  fm_animate_line_chart(scene, y_values, end_value_label=None, accent_color=BRAND_GREEN, x_labels=None, duration=3.5, title_text="")
    Axes-based trend line + gradient fill under curve.
  ...
  fm_animate_waterfall(scene, steps, duration=4.5)
    steps = [{"label": str, "value": float, "color": hex_optional}, ...]. Last step = net total.
  ...
  fm_animate_icon_grid(scene, total, filled, label_text, accent_color=BRAND_GREEN, duration=3.0, cols=10, position=None)
    Crowd grid for population stats. Shows pct hero text beside grid.
    Example: fm_animate_icon_grid(self, 100, 67, "Living Paycheck to Paycheck", BRAND_RED, duration=3.5)
  fm_animate_stacked_cards(scene, items, duration=4.0)
    items = [(label, value_str, color), ...]. Cards slide in from right sequentially. Does NOT take a spacing kwarg -- spacing is fixed internally.
  ...
  fm_animate_bullet_chart(scene, actual, target, range_low, range_high, label_text, accent_color=BRAND_GREEN, duration=3.0, position=None, bar_length=8.0)
    Gray range band + target tick + growing actual bar. For "hitting the target?" beats.
    Example: fm_animate_bullet_chart(self, 500, 1000, 0, 1500, "Monthly Side Income vs Target", BRAND_GOLD, duration=3.5)
  fm_animate_data_table(scene, headers, rows, duration=4.0, header_color=BRAND_GOLD, accent_row=None, accent_color=BRAND_RED, label_text=None, title_text=None, position=None)
    Clean multi-column/multi-row data table -- headers is a list of column names, rows is a list of row-lists (same length as headers). accent_row, if given, is the 0-based row index to highlight (e.g. to call out the row a beat is specifically about). Use for any beat comparing 3+ items across 2+ attributes (e.g. loan options by rate/term/payment) where individual cards or a bar chart would be too cramped or lose the side-by-side comparison.
    Example: fm_animate_data_table(self, ["Plan","Rate","Payment"], [["Fixed","6.5%","$1,850"],["ARM","5.2%","$1,620"]], duration=4.0, accent_row=1)
  fm_icon(name, size=1.0, color=BRAND_GOLD)
    Pure geometry finance icon — NO SVGMobject. Returns a VGroup (single value, not a tuple). Position with .move_to() then self.play(FadeIn(icon)).
  ...
  fm_animate_glow_reveal(scene, text_str, accent_color=BRAND_WHITE, duration=3.0, font_size=88, subtitle=None, subtitle_color=None)
    Dramatic text with expanding glow rings — chapter titles, major reveals, hook moments.
  ...
  fm_animate_single_value(scene, value_str, label_text, accent_color=BRAND_GOLD, duration=3.0, value_size=140, label_size=38, sublabel=None)
    Single hero number with label — for beats with one key figure and no comparison needed. Auto-varies its layout call to call, same as fm_animate_counter.
  ...
  fm_formula(scene, lines="", font_size=60, color=BRAND_WHITE, duration=3.0)
    Plain-Text() formula/calculation display, auto-scaled to ALWAYS fit inside the 16:9 frame regardless of string length — the ONLY way to show a formula, never a raw Text() call. lines is a string or a list of strings (one row each).
  ...
  fm_animate_line_chart_multi(scene, series, duration=4.0, title_text="")
    Two or more trend lines sharing ONE Axes for direct comparison — the ONLY safe way to compare trends, never two separate fm_animate_line_chart calls or a raw Axes. series is a list of {"y_values": [...], "label": str, "color": hex} dicts.
  ...
  fm_animate_vector(scene, direction, label_text, accent_color=BRAND_GOLD, duration=3.5, origin=None, scale=2.5, show_components=False, position=None)
    A single labeled arrow from an origin point in a given direction -- direction is a 2-value [x, y] list. Rarely the right choice for finance content specifically; only reach for this for beats that are genuinely about direction/magnitude together (e.g. a risk-vs-reward vector), not as a substitute for arrow_up/arrow_down icons.
    Example: fm_animate_vector(self, [3, 2], "Risk vs Reward", BRAND_GOLD, duration=3.5)
  fm_animate_matrix(scene, rows_data, label_text="", accent_color=BRAND_GOLD, duration=4.0, cell_size=0.9, font_size=36)
    A labeled grid of numbers -- rows_data is a list of row-lists, all the same length. Rarely needed for finance content; fm_animate_data_table is almost always the better fit when the numbers have column headers or row labels, since a bare matrix has neither.
    Example: fm_animate_matrix(self, [[1200, 1350], [980, 1100]], label_text="Monthly Comparison", duration=3.5)
...
- NEVER index the return value of an fm_animate_* function directly: `gauge = fm_animate_gauge(...); icon.next_to(gauge[1], UP)` crashes with IndexError because fm_animate_gauge returns a 2-tuple (group, val_lbl), not a subscriptable VGroup on its own. Unpack to named variables first: `group, val_lbl = fm_animate_gauge(...)`, then reference `val_lbl` by name. Even better: just call fm_animate_gauge as a bare statement with no assignment if you do not need the returned references at all.

RETURN-VALUE UNPACKING: every fm_animate_* function returns exactly (group, primary_mobject) -- always 2 values, no exceptions, no per-function variation. `group, counter = fm_animate_counter(...)` is always correct arity for every function in this library. If you do not need either returned value, do not unpack at all -- just call the function as a bare statement: `fm_animate_donut(self, 67, "Label", BRAND_RED, duration=3.2)` with no assignment, which is always safe.
